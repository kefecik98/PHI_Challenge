...
#     layer.trainable = True
#  model.compile(optimizer=SGD(lr=0.0001, decay=1e-6, momentum=0.9, nesterov=True), loss='categorical_crossentropy', metrics=['accuracy'])
  model.compile(optimizer=Adam(lr=0.001, beta_1=0.9, beta_2=0.999, epsilon=None, decay=0.9, amsgrad=False), loss='categorical_crossentropy', metrics=['accuracy'])
...
def train(args):
  ...
  filepath = "{val_acc:.4f}_{val_loss:.4f}_{epoch:02d}_"
  checkpoint = ModelCheckpoint("models/t7_{}inres2.model".format(filepath, monitor=['val_acc', 'val_loss'], verbose=1, save_best_only=True, mode='max'))
  ...
  history_tl = model.fit_generator(
    ...
    validation_steps=steps_per_epochv,
...
if __name__=="__main__":
  ...
  a.add_argument("--train_dir", default="Task7/train_dir/")
  a.add_argument("--val_dir", default="Task7/val_dir/")
